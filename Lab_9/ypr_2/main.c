...
int saveArray(struct car_type *a, int size,FILE *f)
{
	fseek(f,0,SEEK_SET);
	fwrite(a, sizeof(struct car_type), size, f);
	return 0;
}

int loadArray(struct car_type *a, int size,FILE *f)
{
	fread(a, sizeof(struct car_type), size, f);
	return 0;
}
...
int main(int argc, char *argv[]) 
{
	...
	{
		printf("cant find file FILE_CARS\n"); return;
	}
